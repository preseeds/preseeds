...
const CreateForm = () => {
  ...
  const fetchOneIdLinkedWallet = async () => {
    ...
    try {
      ...
      const address = await oneId.getWalletsByID(feeReceiver, CHAIN_TYPE.TOMO);
      if (address) {
  ...
  }, [feeReceiver, fetchOneIdLinkedWallet]);
...
return (
  <div className="lg:col-span-7 w-full sm:w-3/4 md:w-2/3 lg:w-1/2">
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-lg font-semibold" htmlFor="targetLiquidity">
      ...
      <div>
        <label className="block text-lg font-semibold" htmlFor="feeReceiver">
          Fee Receiver (Creator will receive 2% of raised VIC if pool is created){" "}
      ...
      {addressReceiver && (
        <div>
          <label className="block text-lg font-semibold" htmlFor="addressReceiver">
