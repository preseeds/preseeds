...
const TokenInfo: React.FC<Props> = ({ tokenAddress }) => {
  ...
  const progressPercentage = targetLiquidity > 0 ? (totalRaised / targetLiquidity) * 100 : 0;
  const twitterShareMessage = `${tokenData[0]} is raising at pre seeds round, all the funds are protected by Preseeds protocol. \n\nSend VIC to: ${tokenAddress} or via Coin98 Magic Link: ${requestUrl} to buy.\n\n Token ${tokenData[1]} will be minted directly to your wallet.`;
          ...
          <div className="flex flex-col md:flex-row md:items-center md:space-x-8">
            <div className="flex-shrink-0 w-full md:w-80 mb-6 md:mb-0">
              {!isError ? (
                  ...
                  >
                    <source src={storage.resolveScheme(tokenData[2])} type="video/mp4" />
                  </video>
              )}
            </div>
              ...
              {tokenData[9] ? (
                  ...
                  <div className="mt-6">
                    ...
                    <div className="text-sm text-gray-400 mt-2">
                      {progressPercentage.toFixed(2)}% raised ({totalRaised} VIC out of {targetLiquidity} VIC)
                    </div>
                  </div>
              )}
            </div>
